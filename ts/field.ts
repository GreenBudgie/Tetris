...
export default class Field {
  ...
  private drawSections(context: CanvasRenderingContext2D): void {
    ...
    for(let x_section: number = 0; x_section <= this.sections_x; x_section++) {
      context.moveTo(x_section * this.section_size_pixels + 0.5, 0);
      context.lineTo(x_section * this.section_size_pixels + 0.5, this.getRealFieldHeight());
    }
    for(let y_section: number = 0; y_section <= this.sections_y; y_section++) {
      context.moveTo(0, y_section * this.section_size_pixels + 0.5);
      context.lineTo(this.getRealFieldWidth(), y_section * this.section_size_pixels + 0.5);
    }
    context.closePath();
    context.stroke();
  }
